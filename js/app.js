...
function confirmDialog(msg, { danger = false, okText = 'OK' } = {}) {
  return new Promise(resolve => {
    const close = openModal({
      title: 'Bestätigen',
      // pre-line, damit mehrzeilige Meldungen (z.B. Vorschaulisten) ihre
      // Zeilenumbrueche behalten - esc() laesst \n stehen, HTML wuerde sie sonst
      // zu Leerzeichen zusammenfalten.
      body: `<p style="margin:0 0 4px;white-space:pre-line">${esc(msg)}</p>`,
...
function updateTabs(hash) {
  ...
  else if (hash.startsWith('/cardio')) active = 'cardio';
...
function renderExerciseBlock(s, id, container, ei) {
  ...
  entry.sets.forEach((set, si) => {
    // Vorbelegung EINMAL bestimmen und sowohl für die Platzhalter-Anzeige als auch
    // für die Übernahme beim Abhaken verwenden. Beides lief vorher auseinander:
    // angezeigt wurden "letztes Mal ODER Ziel-Wdh.", übernommen wurde aber nur
    // "letztes Mal". Wer eine Übung zum ersten Mal machte (oder heute mehr Sätze
    // als beim letzten Mal), sah als Platzhalter z.B. 10, bekam beim Abhaken aber
    // einen leeren Wert gespeichert - und weil isWorkingDone() Wdh. > 0 verlangt,
    // zählte der Satz danach in KEINER Statistik mit, obwohl er abgehakt aussah.
    const lastSet = last?.entry.sets[si];
    const preWeight = lastSet ? DB.num(lastSet.weight) : 0;
    const preReps = lastSet ? DB.num(lastSet.reps) : DB.num(entry.targetReps);
    const phWeight = preWeight > 0 ? fmtWeight(preWeight) : (entry.unit || 'kg');
    const phReps = preReps > 0 ? preReps : '';

    const row = el('div', { class: 'set-row' + (set.done ? ' done' : '') });
    row.innerHTML = `
      <div class="setno">${si + 1}</div>
      <input type="text" inputmode="decimal" placeholder="${esc(phWeight)}" value="${set.weight === '' ? '' : esc(set.weight)}" data-w />
      <input type="text" inputmode="numeric" placeholder="${esc(phReps)}" value="${set.reps === '' ? '' : esc(set.reps)}" data-r />
      <button class="set-check ${set.done ? 'on' : ''}" data-check>✓</button>`;
    wireNumInput($('[data-w]', row), { decimal: true }, v => { set.weight = v; DB.save(); });
    wireNumInput($('[data-r]', row), { decimal: false }, v => { set.reps = v; DB.save(); });
...
route('/cardio', () => {
  setChrome({ title: 'Kardio', back: false, actions: [actionBtn('➕', () => editCardioPlanModal(null)), actionBtn('⚙️', () => manageCardioEquipmentModal())] });
  ...
  // ---------- Statistik ----------
  const totalFinished = DB.cardioSessions().filter(s => s.finishedAt).length;
  ...
  html += `<details class="stats-acc" data-acc="overview" ${cardioStatsAccOpen.overview ? 'open' : ''}><summary>Kardio-Statistik</summary>
    <div class="btn-row" style="margin-bottom:12px">
      <button class="btn ${cardioStatsPeriod === 7 ? 'primary' : 'ghost'} sm" data-cperiod="7">7 Tage</button>
      <button class="btn ${cardioStatsPeriod === 30 ? 'primary' : 'ghost'} sm" data-cperiod="30">30 Tage</button>
      <button class="btn ${!cardioStatsPeriod ? 'primary' : 'ghost'} sm" data-cperiod="0">Alle</button>
    </div>
    <div class="streak-row">
      <div class="stat-tile"><div class="v">${totalFinished}</div><div class="l">Einheiten gesamt</div></div>
      <div class="stat-tile"><div class="v">${periodCount}</div><div class="l">Einheiten (${cardioStatsPeriod ? cardioStatsPeriod + ' Tage' : 'alle'})</div></div>
    </div>
  ...
  // ---------- Letzte Kardio-Trainings ----------
  const recentSessions = DB.cardioSessions().filter(s => s.finishedAt).slice(0, 10);
  html += `<details class="stats-acc" data-acc="recent" ${cardioStatsAccOpen.recent ? 'open' : ''}><summary>Letzte Kardio-Trainings</summary>`;
  html += recentSessions.length
    ? recentSessions.map(cardioSessionRow).join('')
    : `<div class="tiny muted center" style="padding:12px">Noch keine abgeschlossenen Kardio-Trainings.</div>`;
  html += `</details>`;
...
// Formatiert Pace (Minuten pro km) als "5:30/km" statt Dezimalzahl.
...
function editCardioPlanModal(id) {
  const p = id ? DB.getCardioPlan(id) : { name: '', emoji: '🏃', color: COLORS[3] };
  openModal({
    title: id ? 'Kardio-Plan bearbeiten' : 'Neuer Kardio-Plan',
    body: `
      <label class="field"><span>Name</span><input id="f-name" value="${esc(p.name)}" placeholder="z.B. Ausdauer" /></label>
      ${symbolField(p.emoji, 'plan')}
      ${colorField(p.color)}`,
    footer: `${id ? '<button class="btn danger" data-del>Löschen</button>' : ''}<button class="btn ghost" data-x>Abbrechen</button><button class="btn primary" data-ok>Speichern</button>`,
    onMount: (m, close) => {
      ...
      $('[data-ok]', m).onclick = () => {
        ...
        if (!name) return toast('Bitte einen Namen eingeben');
        const data = { name, emoji: sc.getEmoji() || '🏃', color: sc.getColor() };
        if (id) { DB.updateCardioPlan(id, data); close(); render(); }
        else { DB.addCardioPlan(data); close(); render(); }
      };
      const del = $('[data-del]', m);
      if (del) del.onclick = async () => {
        if (await confirmDialog('Kardio-Plan inkl. Trainingstage löschen?', { danger: true, okText: 'Löschen' })) {
          DB.deleteCardioPlan(id); close(); navigate('/cardio');
        }
      };
    },
  });
...
route('/cardio-plan/:id', ({ id }) => {
  ...
  setChrome({ title: `${p.emoji} ${p.name}`, back: true, actions: [actionBtn('➕', () => editCardioDayModal(null, id)), actionBtn('📈', () => navigate('/cardio-stats-plan/' + id)), actionBtn('✏️', () => editCardioPlanModal(id))] });
...
function editCardioDayModal(id, planId) {
  const day = id ? DB.getCardioDay(id) : { name: '', emoji: '🏃', color: COLORS[3], planId };
  openModal({
    title: id ? 'Trainingstag bearbeiten' : 'Neuer Trainingstag',
    body: `
      <label class="field"><span>Name</span><input id="f-name" value="${esc(day.name)}" placeholder="z.B. Grundlagenausdauer" /></label>
      ${symbolField(day.emoji, 'day')}
      ${colorField(day.color)}`,
    footer: `${id ? '<button class="btn danger" data-del>Löschen</button>' : ''}<button class="btn ghost" data-x>Abbrechen</button><button class="btn primary" data-ok>Speichern</button>`,
    onMount: (m, close) => {
      ...
      $('[data-ok]', m).onclick = () => {
        ...
        if (!name) return toast('Bitte einen Namen eingeben');
        const data = { name, emoji: sc.getEmoji() || '🏃', color: sc.getColor() };
        if (id) { DB.updateCardioDay(id, data); close(); render(); }
        else { const nd = DB.addCardioDay({ ...data, planId: day.planId }); close(); navigate('/cardio-day/' + nd.id); }
      };
      const del = $('[data-del]', m);
      if (del) del.onclick = async () => {
        if (await confirmDialog('Trainingstag löschen?', { danger: true, okText: 'Löschen' })) {
          const pid = day.planId; DB.deleteCardioDay(id); close(); navigate('/cardio-plan/' + pid);
        }
      };
    },
  });
...
route('/cardio-day/:id', ({ id }) => {
  const day = DB.getCardioDay(id);
  ...
  openModal({
    ...
    onMount: (m, close) => {
      ...
      $('[data-ok]', m).onclick = () => {
        const name = $('#f-eqname', m).value.trim();
        if (!name) return toast('Bitte einen Namen eingeben');
        const eq = DB.addCardioEquipment(name);
        close(); onAdded(eq.name);
      };
      $('#f-eqname', m).focus();
    },
  });
...
function manageCardioEquipmentModal() {
  let editingId = null;
  function draw(m) {
    const list = DB.cardioEquipmentTypes();
    const listEl = $('#ceqList', m);
    listEl.innerHTML = list.length ? list.map(e => {
      const uses = DB.cardioEquipmentUsage(e.id);
      if (editingId === e.id) {
        return `<div class="list-row" data-eq="${e.id}">
          <input id="f-rename-${e.id}" value="${esc(e.name)}" style="flex:1" />
          <button class="btn ghost sm" data-save="${e.id}">✓</button>
...
function cardioPaceText(entry) {
  const durationMin = DB.num(entry.durationMin), km = DB.num(entry.km);
  if (durationMin > 0 && km > 0) {
    const speed = Math.round(km / (durationMin / 60) * 10) / 10;
    return fmtPace(durationMin / km) + ' · ' + speed + ' km/h';
  }
  return '–';
}
function cardioEntryFieldsHTML(entry) {
  return `
    ...
    <div class="row3">
      <label class="field"><span>Dauer (Min.)</span><input type="text" inputmode="numeric" value="${entry.durationMin === '' ? '' : esc(entry.durationMin)}" data-f="durationMin" /></label>
      <label class="field"><span>Distanz (km)</span><input type="text" inputmode="decimal" data-dec value="${entry.km === '' ? '' : esc(entry.km)}" data-f="km" /></label>
      <label class="field"><span>Herzfrequenz</span><input type="text" inputmode="numeric" value="${entry.heartRate === '' ? '' : esc(entry.heartRate)}" data-f="heartRate" /></label>
    </div>
    <div class="row3">
      <label class="field"><span>Zone</span><input type="text" inputmode="numeric" value="${entry.zone === '' ? '' : esc(entry.zone)}" data-f="zone" /></label>
      <label class="field"><span>Stufe</span><input type="text" inputmode="numeric" value="${entry.level === '' ? '' : esc(entry.level)}" data-f="level" /></label>
      <label class="field"><span>Kalorien</span><input type="text" inputmode="numeric" value="${entry.calories === '' ? '' : esc(entry.calories)}" data-f="calories" /></label>
    </div>
    <div class="row3">
      <label class="field"><span>Watt</span><input type="text" inputmode="numeric" value="${entry.watt === '' ? '' : esc(entry.watt)}" data-f="watt" /></label>
      <label class="field"><span>Höhenmeter</span><input type="text" inputmode="numeric" value="${entry.elevationM === '' ? '' : esc(entry.elevationM)}" data-f="elevationM" /></label>
      <div class="field"><span>Pace / km/h</span><div class="tiny muted" data-pace style="padding-top:8px">${cardioPaceText(entry)}</div></div>
    </div>
  `;
}
function wireCardioEntryFields(block, entry, onChange) {
  // Textfelder mit Normalisierung statt type="number" - siehe normalizeNumInput:
  // ein getipptes Komma wuerde sonst stillschweigend verschluckt und z.B. aus
  // "5,3 km" die Distanz "53 km" machen. Nur die Distanz ist ein Dezimalfeld.
  $$('[data-f]', block).forEach(inp => {
    const decimal = inp.hasAttribute('data-dec');
    wireNumInput(inp, { decimal }, v => {
      entry[inp.dataset.f] = v;
      if (inp.dataset.f === 'durationMin' || inp.dataset.f === 'km') {
        const paceEl = $('[data-pace]', block);
        if (paceEl) paceEl.textContent = cardioPaceText(entry);
      }
...
route('/cardio-session/:id', ({ id }) => {
  ...
  function draw() {
    ...
    $$('[data-entry]', appEl).forEach((block, i) => {
      ...
      block.innerHTML = cardioEntryFieldsHTML(entry);
      wireCardioEntryFields(block, entry, () => DB.save());
    });
  }
  draw();
});
...
// Deutsche Tastaturen liefern ein KOMMA als Dezimaltrenner. Ein
// <input type="number"> verwirft das Komma stillschweigend, statt es zu
// übernehmen oder abzulehnen: aus getipptem "77,5" wurde "775" - ein zehnfach zu
// hoher Wert, der plausibel aussieht und das Volumen massiv verfälscht. Deshalb
// sind die Zahlenfelder Textfelder mit inputmode (gleiche Tastatur auf dem Handy)
// und eigener Normalisierung.
function normalizeNumInput(raw, { decimal = true } = {}) {
  let v = String(raw ?? '').replace(',', '.').replace(/[^0-9.]/g, '');
  if (!decimal) return v.replace(/\./g, '');
  const i = v.indexOf('.');
  if (i >= 0) v = v.slice(0, i + 1) + v.slice(i + 1).replace(/\./g, ''); // nur EIN Trennzeichen
  return v;
}

// Hängt die Normalisierung an ein Eingabefeld. Der Feldinhalt wird nur dann
// zurückgeschrieben, wenn wirklich etwas entfernt wurde - sonst würde der Cursor
// bei jedem Tastendruck ans Ende springen.
function wireNumInput(input, { decimal = true }, onValue) {
  input.oninput = e => {
    const v = normalizeNumInput(e.target.value, { decimal });
    if (v !== e.target.value) {
      const atEnd = e.target.selectionStart === e.target.value.length;
      e.target.value = v;
      if (!atEnd) { try { e.target.setSelectionRange(v.length, v.length); } catch (_) { /* egal */ } }
    }
    onValue(v);
  };
}
    ...
    onMount: (m, close) => {
      ...
      function draw() {
        ...
        if (tab === 'ex') {
          ...
          listEl.innerHTML = items.length ? items.map(s => `<div class="list-row">
              <div class="grow"><div class="r-title">${esc(s.dayName || 'Kardio-Training')}</div><div class="r-sub">${fmtDate(s.date)} · gelöscht am ${fmtTs(s.deletedAt)}</div></div>
              <button class="btn ghost sm" data-restorecses="${s.id}" title="Wiederherstellen">↩️</button>
              <button class="btn danger sm" data-purgecses="${s.id}" title="Endgültig löschen">🗑️</button>
            </div>`).join('') : `<div class="tiny muted center" style="padding:12px">Papierkorb leer.</div>`;
        }
        ...
        $$('[data-purgecses]', listEl).forEach(b => b.onclick = async () => {
          if (await confirmDialog('Endgültig löschen?', { danger: true, okText: 'Löschen' })) { DB.purgeTrashedCardioSession(b.dataset.purgecses); draw(); render(); }
        });
      }
...
route('/settings', () => {
  ...
  appEl.innerHTML = `
    ...
    <div class="card">
      ...
      <div class="tiny muted">${stats.loc} Orte · ${stats.plans} Pläne · ${stats.days} Trainingstage · ${stats.ex} Übungen · ${stats.ses} Einheiten · ${DB.cardioSessions().length} Kardio-Einheiten</div>
      ...
      <label class="field" style="margin:0;display:flex;align-items:center;gap:10px">
        ...
        <span style="margin:0">Kraft-Beispieldaten (ca. 2 Monate Testtrainings)</span></label>
      ...
      <label class="field" style="margin:12px 0 0;display:flex;align-items:center;gap:10px">
        <input id="s-cdemo" type="checkbox" ${DB.hasCardioDemoSessions() ? 'checked' : ''} style="width:auto" />
  ...
  $('#s-cdemo', appEl).onchange = e => {
    if (e.target.checked) {
      const n = DB.generateCardioDemoSessions();
      if (!n) { toast('Braucht mind. einen Kardio-Trainingstag mit Aktivitäten'); e.target.checked = false; return; }
      toast(n + ' Kardio-Beispiel-Trainings erzeugt');
    } else {
      DB.removeCardioDemoSessions();
      toast('Kardio-Beispieldaten entfernt');
    }
  };
