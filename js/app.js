...
route('/', () => {
  ...
  const recent = DB.sessions().filter(s => s.finishedAt && s.locationId === locId).slice(0, 5);
  ...
  if (!unfinished.length && !recent.length) {
    html += `<div class="empty">
      <div class="big">💪</div>
      <div>Bereit für dein nächstes Training?</div>
      <div class="tiny" style="margin:8px 0 14px">Wähle unter „Pläne" einen Trainingstag zum Starten.</div>
      <a class="btn primary" href="#/plans">Zu den Plänen</a>
    </div>`;
  }
